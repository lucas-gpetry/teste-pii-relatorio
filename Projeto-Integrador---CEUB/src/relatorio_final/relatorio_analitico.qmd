...
format:
  html:
    ...
    theme: cosmo       # ou outro tema Quarto
    css: style.css    # opcional, se quiser customizar fontes/cores
    include-in-header:
      text: |
...
# Para HTML, cabeçalhos e rodapés em LaTeX não são usados
# Você pode fazer isso via CSS ou HTML diretamente
---
<div class="header-custom">
  <h1>Relatório Analítico — Violência contra a Mulher</h1>
  <img src="img_maps/pmdf.png" alt="Logo">
</div>
...
```{python}
from reports.report_theme import set_report_theme
from reports.report_plots import *
import pandas as pd
import numpy as np
...
from reports.report_plots import bar_horizontal_highlight
...
[Ver mapa de calor](img_maps/mapa_1_batalhoes.html){.botao-mapa target="_blank"}

## O Cruzamento com Feminicídios
...
[Ver mapa de calor](img_maps/mapa_crime_vs_batalhoes.html){.botao-mapa target="_blank"}
...
Se alimentados com a base histórica completa (últimos 5 anos), os algoritmos poderão prever estatisticamente em quais quadrantes (Células de 500m) ocorrerão os próximos crimes no turno subsequente, permitindo à PMDF migrar de um modelo de patrulhamento de ronda para um **policiamento preditivo de precisão**.